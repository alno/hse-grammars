...
/* Здесь рассматривается разбор с помощью формализма
 * Parsing Expression Grammars, который очень похож на контекстно-свободные грамматики.
 * 
 * Грамматика состоит из набора правило, каждое из которых связывает некоторый нетерминал с выражением,
 * в которое входят терминалы, нетереминалы и знаки операций.
 * 
 * Основными операциями являются:
 * - Последовательное применение
 * - Альтернативное применение
 * - Опциональное вхождение
 * - Повторение ноль или большее число раз
 * - Повторение один или большее число раз
 * В данной программе для простоты используются только первые две операции
 * 
 * Каждое выражение описывает некоторый парсер, который принимает входную строку и возвращает
 * флаг успешности разбора и неразобранный остаток строки.
 * 
 * За счет использования операций можно определять более сложные парсеры на основе более простых.
 * 
 * Эти грамматики интересны тем, что такой подход позволяет реализовать набор операций в языке
 * таким образом, чтобы описывать грамматики непосредственно в коде.
 */
